...
folds_dir = "/DATA2/akshay/Akshat/sample_disjoint_datalist/mutispectral"
embedding_cache_path = "warsaw_openclip_embeddings_multispectral.npz"
...
def extract_multispectral_features(df, nir_dir, rgb_dir, batch_size=32):
    ...
    for i in tqdm(range(0, len(df), batch_size), desc="Extracting MultiSpectral OpenCLIP Features"):
        batch_tensors = []
        ...
        for _, row in df.iloc[i:i+batch_size].iterrows():
            ...
            try:
                nir_img = Image.open(nir_path).convert('L').convert('RGB')  # Make it 3-channel
                rgb_img = Image.open(rgb_path).convert('RGB')
                nir_tensor = transform(nir_img)
                rgb_tensor = transform(rgb_img)
                combined_tensor = (nir_tensor + rgb_tensor) / 2
            except Exception as e:
                print(f"Error loading image {row['nir_filename']} or {row['rgb_filename']}: {e}")
...
for fold in range(1, 11):
    ...
    joblib.dump(mlp, f"mlp_model_openclip_multispectral_fold{fold}.joblib")
    joblib.dump(scaler, f"scaler_openclip_multispectral_fold{fold}.joblib")
